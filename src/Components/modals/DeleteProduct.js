...
export default function DeleteProduct({ handleClose, open, data, getData, setData }) {
  const [Loading, setLoading] = React.useState(false);
  const [Open2, setOpen] = React.useState(false);
  const _DeleteUser = async (uid) => {
    setData(items => items.filter(it => it?._id != uid))
    setLoading(true);
    const api = await deleteProducts(uid)
    if (api.status === 200) {
      setLoading(false);
      handleClose()
      handleModal2();
    ...
    <div>
      <Modal
        ...
        <Box sx={{ ...style, width: 470 }}>
          ...
          <Paragraph>Are you sure you want to delete this NFTs?</Paragraph>
          {/* <Paragraph>
            delete Product <Name> “{data.name}”</Name>?
          </Paragraph> */}
          <Row>
      ...
      <DeletedUni handleClose={handleModal2} open={Open2} getData={getData} />
    </div>
  );
}

function DeletedUni({ handleClose, open, getData }) {
  function close() {
    handleClose();
    // getData();
  }
    ...
    <div>
      <Modal
        ...
        <Box sx={{ ...style, width: 450 }}>
          ...
          <Paragraph>Product Deleted</Paragraph>
          <Row>
