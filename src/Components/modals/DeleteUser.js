...
export default function DeleteUser({
  ...
  const handleDeleteCelebrity = async (currentCelebrityId) => {
    setLoading(true);
    const response = await DeleteSingleData(
      `api/celebrity/delete/${currentCelebrityId}`
    );
    ...
    if (response?.status) {
      ...
      setLoading(false);
      alert("celebrity deletion failed");
      handleClose();
    }
  };
    ...
    <div>
      ...
      >
        <Box sx={{ ...style, width: 470 }}>
          ...
          <img
            src={Vector}
            style={{ marginBottom: "30px", height: "75px" }}
            alt="logo"
          />

          <Paragraph>Are you sure you want to delete this celebrity</Paragraph>
          {/* <Paragraph>
            delete user <Name> “{ data && data?.name}”</Name>?
          </Paragraph> */}
          <Row>
            <Discard disabled={Loading} onClick={()=>handleDeleteCelebrity(currentCelebrityId)}>
              Delete
            </Discard>
    ...
    <div>
      ...
      >
        <Box sx={{ ...style, width: 450 }}>
          ...
          <Paragraph>User Deleted</Paragraph>
          <Row>
