...
const AddRecentVideo = () => {
  ...
  const handleAddNewCelebrityVideo = async (e) => {
    e.preventDefault();
    try {
      setLoader(true);
      ...
      UploadImage({ img: tempimg }).then(async (res) => {
        ...
        const response = await PostData(
          `api/video/addNewCelebrityVideo`,
          { ...data, celebrityVideo:res, celebrityID:params?.celebrityid }
        );
        ...
        <form style={{ width: "97%" }}>
          ...
          >
            <ButtonComponent
              ...
              name="Add Video"
              dashboard={true}
              onClick={handleAddNewCelebrityVideo}
            />
          </Grid>
        </form>
      </div>
